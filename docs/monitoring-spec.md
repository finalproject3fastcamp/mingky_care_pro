...
**감사 로그** — `system_stop` · `estop` · teleop 을 **누가** 실행했는지 남지
않는다. 병원 도메인에서는 기능이 아니라 요건이고, §1.1 의 "수동 개입 없음"
판정에도 필요하다. `orders` 에 actor 를 추가하고 "최근 수동 개입" 목록을 둔다.
...
모든 개입은 SLO 위반으로 집계된다(§1.1). 개입 기록이 남지 않으면 SLO 를 측정할
수 없으므로 감사 로그가 선행 조건이다.
...
- 프론트·백엔드 담당자가 **로봇 대기 없이** 4대 군집을 띄우고 개발한다.
  6명 중 절반이 하드웨어 큐에서 빠진다
- 실패 경로를 마음대로 만든다. `comm_lost` 를 보려고 Wi-Fi 를 끊을 필요가 없다.
  끼임·배터리 방전·pick 연속 실패처럼 실기 재현이 어렵거나 위험한 것들
- 그대로 **통합 테스트 픽스처**가 된다. CI 에서 가짜 로봇 4대를 띄워 세션 완주를
  검증하면 지금 단위 테스트 13개가 못 잡는 배관 오류가 잡힌다
- **`arm.*` 를 팔 실기 없이 먼저 설계·검증**할 수 있다 (§6.2)
- 발표용 데모 모드가 공짜로 따라온다

시나리오는 YAML 로 두고 `event_codes.yaml` 의 payload 정의를 참조하게 한다.
...
| # | 항목 | 근거 |
| --- | --- | --- |
| ~~2~~ | ~~워커 1개 가드 (§9.2)~~ | **완료** — 파일 락. `test_worker_guard.py` |
| ~~3~~ | ~~`robot_types` 메타 + ingest 검증 (§6.1)~~ | **완료** — `010` 명명 정리 포함. `test_ingest.py` |
| 1 | **가짜 로봇 하네스** (§9.1) | 반나절. 이후 모든 항목의 개발 속도를 올린다 |
| 4 | 감사 로그 (actor) (§7.2) | **SLO 측정의 선행 조건.** 이게 없으면 §1.1 판정 불가 |
| 5 | 세션 완주율 집계 + `fleet` 탭 | SLO 를 눈에 보이게 |
| 6 | `arm.*` 이벤트 코드 + 팔 게이트웨이 연결 (§6.2) | 조제 로봇 2대가 관제에 보이지 않는다. 1번이 있으면 실기 없이 진행 |
| 7 | `system` 탭 타입 분기 + 팔 패널 (§7.3) | 팔을 관제 안으로 |
| 8 | CI 게이트 (§8.1) | 1번이 있으면 통합 테스트까지 포함 가능 |
| 9 | 토픽 주기(Hz) 감시 (§7.2) | 못 잡는 장애 모드를 잡음 |
| 10 | 형상 패널 (SHA · 맵 해시 · 체크포인트) | 재현성 문제의 대부분 |
| 11 | 서보 온도·전류 수집 (§4.4) | 유일한 실질 예지보전 신호 |
| 12 | 알림 웹훅 (§8.4) | 원칙 4 |

**남은 것 중 1번을 먼저 한다.** 2·3번은 로봇 실기 없이 끝낼 수 있어 먼저
처리했다. 6·8번은 가짜 로봇이 서야 진행되므로 1번이 선행이다.

4번이 5번의 선행 조건이라는 점에 주의한다. 누가 개입했는지 기록되지 않으면
SLO 를 측정할 방법이 없다.
