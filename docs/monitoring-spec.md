...
| 형상 | git SHA, 부팅 경과 | 맵 격자 지문 | **정책 체크포인트 · 데이터셋 revision** |
...
**Dynamixel 이 공짜로 주는 신호가 있다 — 구현됨.** 서보가 온도·전류·전압·
하드웨어 에러 비트를 직접 보고한다. mobile 에 없는 진짜 예지보전 신호다.
`POST/GET /robots/{id}/servos` · `robot_servo_log`(012) · `app/servo_health.py`
· `ServoHealthCard`.

**지금 뜨거운 것과 오르는 중인 것을 따로 판정한다.** 40℃ 인데 회차마다 오르는
조인트가 55℃ 에서 평평한 조인트보다 나쁜 신호다 — 전자는 그리퍼 마모나
과부하 자세이고 후자는 그냥 그런 축이다. 하나로 뭉개면 온도계 하나만 남는다.

추세는 함부로 내지 않는다. 표본이 모자라거나(5개 미만) 시간 폭이 짧거나
(15분 미만) 적합이 나쁘면 기울기를 내지 않는다 — 조제는 사이클마다 부하가
출렁여서, 짧은 창의 기울기는 다음 표본에 뒤집힌다. `battery_forecast` 와
같은 규칙이고 최소제곱 함수도 그쪽 것을 그대로 쓴다.

임계는 `config/servo_limits.yaml` 이고 **조인트마다 다르다.** 그리퍼는 쥔 채
버티는 시간이 길어 뜨겁게 도는 것이 정상이라, 공통선을 쓰면 정상 동작이 매
사이클 경고가 된다. XM430 은 80℃ 에서 스스로 토크를 끊으므로 경고선은 그
아래(55/65 초안)에 둔다 — 그 지점에서 처음 알리면 이미 사이클이 깨진 뒤다.

임계 통과는 `manipulator.servo_overheat`(warning)로 나간다. 팔은 아직 돌고
있으므로 `error` 가 아니다(§8.4). 해제(`servo_cooled`)는 경고선까지 내려와야
발행한다 — 히스테리시스가 없으면 임계 근처에서 흔들리는 조인트 하나가
알림을 잡음으로 만든다.
...
| **메트릭** (연속) | 서보 온도·전류 추이 | `robot_servo_log` (012) | 초안 90일 | 팔만 구현됨 |
| **메트릭** (연속) | 시간당 goal 성공률 | 1분 롤업 테이블 | 90일 | **TODO** |
| **로그** (원문) | journald, rosbag | 로봇 로컬 | 트리거 덤프만 | **TODO** |

메트릭 평면의 첫 입주자가 서보 온도·전류다(§4.4, 로드맵 11). 원칙 2 와
충돌하지 않는다 — 여기서 필요한 것은 현재값이 아니라 **추세**이고, 메모리에
두면 서버 재시작마다 사라져 "요즘 나빠지고 있나" 를 영영 못 본다. 로봇 4대
규모에서는 Prometheus 보다 PostgreSQL 표 하나가 싸다(§10).

아직 롤업은 없다. 팔 2대 × 조인트 5개 × 분당 1회면 하루 14,400행이라 원본을
그대로 들고 있어도 문제가 되지 않고, 삭제 주기를 실측 없이 코드에 박으면
그게 정본이 되어 버린다(§13). 주행 쪽 집계(goal 성공률 등)는 여전히 비어 있다.

---

## 6. 이벤트 코드 정본

현재 59개. 영역별로 `qr` `patient` `fire` `nav` `waypoint` `localize` `dock`
...
**형상 패널 — 구현됨.** "이 로봇이 지금 무슨 커밋·무슨 맵·무슨 정책으로
도는가" 를 한눈에 보는 곳이 없었다. 커밋은 인벤토리 안쪽에, 맵은 아무 데도,
정책 체크포인트는 조제 패널에 흩어져 있었다. 데모가 어제는 됐는데 오늘 안
되는 원인의 대부분이 여기다.

`GET /fleet/config`(`app/fleet_config.py`) · `FleetConfigCard`.

- **맵은 파일이 아니라 `/map` 격자를 해싱한다.** 디스크의 yaml 을 읽으면
  "런치 인자가 가리키는 파일" 을 보는 것이고, 그건 map_server 가 실제로 실은
  것과 다를 수 있다. 이름이 아니라 지문으로 비교하는 이유도 같다 — 같은
  이름의 다른 맵이 실제로 있다. 해상도·크기·원점까지 넣는다. 격자가 같아도
  원점이 다르면 좌표가 통째로 밀린 다른 맵이다.
- **타입 안에서만 비교한다.** 팔의 버전은 코드 SHA 가 아니라 정책 체크포인트와
  데이터셋 revision 이다(§4.4). OMX 의 SHA 와 핑키의 SHA 를 나란히 놓으면 항상
  다르고, 항상 빨간 경고는 없는 것과 같다.
- **'다르다' 와 '모른다' 를 구분한다.** OMX 는 게이트웨이가 아직 없어(로드맵 6)
  코드 형상을 정상적으로 보고하지 않는다. 그걸 불일치로 세면 패널이 영구히
  빨갛다. 대신 판정이 **몇 대를 본 것인지**(`unreported`)를 같이 내려보낸다 —
  2대만 비교한 "같다" 를 4대의 "같다" 로 읽으면 안 된다.
- **커밋 안 된 변경(`dirty`)을 드러낸다.** 커밋 해시만으로 재현이 불가능한
  상태이므로, 해시가 같아도 같은 형상이 아니다.
- 맵 지문은 heartbeat 가 아니라 **인벤토리**(변할 때만 오는 층)에 싣고
  `inventory_hash` 에 포함시킨다. 안 넣으면 맵만 바꾼 로봇의 변화가 다음
  재기동까지 서버에 안 보인다.

`scenarios/fleet_config_split.yaml` 이 커밋·맵이 갈린 2대를 재생한다. 실기로
만들려면 로봇 한 대에만 다른 브랜치를 배포하고 재기동해야 한다.
...
- **백엔드** — 공통 필드는 평평하게, 타입별 지표는 `detail` 한 칸에 담는다.
  ...
  서보 온도는 실제로 그 예외가 됐다(로드맵 11). 이벤트로 재구성되지 않는
  연속값이라 `robot_servo_log`(012)에 따로 쌓는다. 다만 `detail` 에는 넣지
  않았다 — `GET /robots` 는 3초 폴링인데 추세는 몇 시간 창의 집계라, 같이
  두면 그 쿼리가 3초마다 돈다. 배터리 예상(`/battery-forecast`)을 따로 뺀
  것과 같은 이유로 `GET /robots/{id}/servos` 다.
...
1. **알림 라우팅 — 구현됨.** 정본은 `config/alert_routes.yaml`, 발송은
   `app/notify.py` 다. `ALERT_WEBHOOK_URL` 이 없으면 꺼진 채로 돈다 — 기본이
   '안 보냄' 이라야 개발·CI 가 실수로 실제 채널에 쏘지 않는다. Slack 과
   Discord 는 본문 키만 다르므로 URL 호스트로 갈라 둘 다 받는다(§13 이
   채널을 미결정으로 남겼고, 결정이 나면 URL 만 바꾸면 된다).

   등급은 둘이다. `page`(즉시 사람 호출)는 `fire.detected` ·
   `robot.estop_engaged` · `robot.comm_lost` **셋이 상한**이고, 넷째를 넣고
   싶으면 기존 하나를 내리는 쪽을 먼저 생각한다. 나머지(배터리·서보 결함·
   과열·사이클 포기·토픽 두절·통신 복구)는 `notify` 다.

   보내는 것보다 **안 보내는 것**이 중요하다. 셋을 막는다.

   | 막는 것 | 왜 |
   | --- | --- |
   | 중복 (같은 로봇·코드 5분) | 두절이 흔들리는 로봇 하나가 채널을 도배한다 |
   | 지난 일 (5분 초과) | 두절 복구 배치의 10분 전 사건이 현재형으로 읽힌다 (§3.2) |
   | 홍수 (배치당 5건) | 몰려 들어온 배치에서 진짜 한 건을 못 찾는다 |

   발송은 **적재 트랜잭션 밖**에서 태스크로 던지고 결과를 안 본다. 웹훅
   응답을 트랜잭션 안에서 기다리면 슬랙이 느린 날 로봇의 이벤트 큐가 밀린다.
   커밋된 사실만, 그리고 **새로 들어온 이벤트만** 내보낸다 — 재전송된 배치가
   같은 사건을 다시 울리면 두절이 한 번 있었을 뿐인데 채널에는 열 번 찍힌다.
2. **심각도 분리** — "즉시 사람 호출" 등급은 손에 꼽게 유지한다. 알림이 흔해지면
   무시된다. 확률적 실패(`manipulator.pick_failed`)는 정본에서 **어느 등급에도
   없다** — 모방학습 pick 성공률은 원래 90% 근처라 하루에 수십 번 울리고,
...
| 로봇 4대 SHA 동일 | **드러남** | 형상 패널이 갈린 축을 표시 (§7.2) |
...
- 알림 채널 (Slack vs Discord) — 둘 다 받게 해뒀다. URL 만 정하면 된다
- QR 서명 토큰 전환 시점
- 팔 SLI 기준선 — 초기 N회 실측이 필요하다. 서보 온도 임계(55/65)도 같은 항목이다
- 서보 로그 보존·롤업 — 지금은 원본을 그대로 쌓고 정리하지 않는다
...
| ~~10~~ | ~~형상 패널 (SHA · 맵 해시 · 체크포인트)~~ | **완료** — `/fleet/config` · `FleetConfigCard`. `test_fleet_config.py` |
| ~~11~~ | ~~서보 온도·전류 수집 (§4.4)~~ | **완료** — `robot_servo_log`(012) · `servo_health.py` · `ServoHealthCard`. `test_servo_health.py` |
| ~~12~~ | ~~알림 웹훅 (§8.4)~~ | **완료** — `alert_routes.yaml` · `notify.py`. Slack·Discord 둘 다. `test_notify.py` |
