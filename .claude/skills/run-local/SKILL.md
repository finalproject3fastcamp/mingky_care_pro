...
- **약국 화면이 열려 있으면 백엔드가 SIGTERM 으로 안 죽는다** — `/pharmacy/progress`
  ...
  반드시 확인할 것.
  → 로봇 러너는 이때도 따라 죽는다. 정상 종료면 `lifespan` 이 SIGINT 를 보내
  토크를 풀고 세우고(로그에 "종료 중 — 로봇 러너를 정리합니다"), SIGKILL 이면
  그 훅이 안 도니 자식에 걸어 둔 `PR_SET_PDEATHSIG` 가 커널 쪽에서 처리한다.
  **팔을 움직이는 자식이 부모보다 오래 사는 일은 없어야 한다** — 감시자 없이
  팔이 돌고, 포트·카메라를 쥔 채라 다음 기동까지 막힌다.
