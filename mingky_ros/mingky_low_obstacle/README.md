...
5. Nav2 local costmap의 `RangeSensorLayer`와 MPPI가 가까운 회피를 수행하고,
   ...
   다시 만듭니다.
6. 15cm 이내에서는 전진 속도만 0.08m/s로 제한하고, **2-of-3 저상 장애물
   확정 이후** 4cm 이내가 반복되면 전진만 막습니다. 회전과 후퇴 명령은
   변경하지 않습니다.
...
10cm 미만의 초근접값은 실제 거리 그대로 찍지 않고 padded footprint 바깥인
10cm로 제한해 표시합니다. 실제 거리 그대로 찍으면 MPPI의 시작 자세가 충돌로
판정되지만, 최대 거리로 지워 버리면 Smac2D가 장애물을 보지 못해 우회 경로를
만들 수 없기 때문입니다. 별도의 속도 게이트는 실제 초음파 거리를 사용합니다.
...
기준 2회 연속 미검출(약 0.2초)을 먼저 확인합니다. 이후에도 costmap 표식은
1.5초 더 유지해 1Hz 전역 planner가 안정된 장애물을 보고 우회 경로를 만들게
합니다. 그 사이 같은 장애물이 재검출되면 삭제 예약을 취소합니다.
또한 단순 전진 접근 중에는 표식을 지우지 않습니다. 로봇이 최초 등록 자세에서
옆으로 10cm 이동하거나 뒤로 10cm 물러나거나 20도 회전해 회피가 확인되면
1.5초 유예 후 과거 부채꼴을 삭제합니다. 이로써 원래 경로와 우회 경로 사이의
반복 전환을 막으면서 동적 물체를 지도에 고정하지 않습니다.
...
확정된 15cm/4cm 장애물은 센서가 잠시 끊겨도 마지막 전진 제한을 유지하지만,
