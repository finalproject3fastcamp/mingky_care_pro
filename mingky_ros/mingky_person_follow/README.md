...
| 상태 | 기본 기준 | 동작 |
| --- | --- | --- |
| `normal` | 1.5m 이하 | 100% 주행 |
| `slow` | 1.5m 초과, 2.5m 미만 | 35% 감속 |
| `waiting` | 2.5m 이상 또는 센서 두절 | Nav2 목표 취소 후 대기 |
| `inactive` | 안내 중이 아님 | 속도 제한 해제 |

경계에서 속도가 흔들리지 않도록 기본 0.2m hysteresis와 최근 QR
거리 median 필터를 사용한다. 실로봇 검증 후 로봇별 거리 임계값을
`robot.env`에서 조정한다.

## 통합 실행

실로봇 재검증 전에는 기본으로 꺼져 있다.
...
| 이름 | 기본값 | 설명 |
| --- | ---: | --- |
| `slow_distance_m` | `1.5` | 감속 시작 거리 |
| `stop_distance_m` | `2.5` | 환자 대기 전환 거리 |
| `distance_hysteresis_m` | `0.2` | 상태 복귀 여유폭 |
...
실로봇에서는 QR 거리가 실측과 맞는지, 감속 구간, 2.5m 대기, 복귀
