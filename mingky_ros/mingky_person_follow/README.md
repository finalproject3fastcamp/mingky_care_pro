...
안내 중인 인형의 QR 목걸이로 환자 ID와 거리를 확인해 Nav2 주행을
정상·감속·대기 상태로 조절한다. QR이 짧게 가려진 동안에만
YOLO 인형 박스로 거리를 보완한다.

## 안전 원칙

- `/guide_manager/state`가 `guiding`이고 QR data가 현재 `patient_id`와
  같을 때만 환자 거리로 인정한다.
- QR 거리가 주 판단이다. YOLO는 마지막 QR 검증 후 제한된
  시간 동안만 쓴다.
- QR과 YOLO 모두 오래되면 계속 주행하지 않고 `waiting`이 된다.
...
/rear_camera/image_raw/compressed
  └─ HTTP YOLO (선택)
       └─ QR 픽셀과 같은 인형 박스를 짧게 추적
```

YOLO는 직전 QR 거리와 박스 높이를 기준으로 삼아 QR이 짧게
가려진 동안의 거리를 근사한다. 보완 시간이 끝나면 박스가 보여도
QR ID를 다시 확인할 때까지 대기한다.
...
3. 같은 프레임의 QR 픽셀 중심에 가장 가까운 YOLO 인형 박스를 환자로
   잠그고, 클래스와 위치가 갑자기 바뀌는 후보는 제외한다.
4. QR이 순간적으로 가려지면 마지막 QR 거리와 그때의 박스 높이를 기준으로
   `현재 거리 = 마지막 QR 거리 × 마지막 박스 높이 / 현재 박스 높이`를
   적용한다. 인형이 멀어져 박스가 작아지면 추정 거리는 커진다.
5. 이 보완은 기본 2초만 허용한다. 그 안에 QR ID를 다시 확인하지 못하면
   다른 인형을 환자로 오인하지 않도록 `waiting`으로 전환한다.

즉 YOLO가 QR의 절대 거리를 새로 측정하는 구조가 아니라, OpenCV QR로
확인한 환자와 거리 기준을 짧은 가림 동안만 이어 주는 구조다.
...
| `slow` | 0.15m 초과, 0.20m 미만 | 35% 감속 |
| `waiting` | 0.20m 이상 또는 센서 두절 | Nav2 목표 취소 후 대기 |
...
| `stop_distance_m` | `0.20` | 환자 대기 전환 거리 |
| `distance_hysteresis_m` | `0.02` | 상태 복귀 여유폭 |
| `qr_stale_sec` | `1.0` | QR 거리 신뢰 시간 |
| `visual_fallback_sec` | `2.0` | 마지막 QR 후 YOLO 보완 허용 시간 |
...
실로봇에서는 QR 거리가 실측과 맞는지, 감속 구간, 0.20m 대기, 복귀
