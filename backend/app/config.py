...
# 토픽 주기 판정 주기 (§7.2). heartbeat 판정과 따로 돈다 — 저쪽은 로봇이 살아
# 있는가이고 이쪽은 살아 있는 로봇 안에서 데이터가 흐르는가라, 한 루프에 묶으면
# 두절 판정 주기를 토픽 사정으로 바꾸게 된다.
TOPIC_WATCH_INTERVAL_SEC = float(os.environ.get("TOPIC_WATCH_INTERVAL_SEC", 5))
